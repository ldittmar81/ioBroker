...
function setup(callback) {
    ...
    recCopy(path.join(rootDir, 'node_modules', 'iobroker'), path.join(rootDir), {overwrite: true}, function(error, results) {
        if (error) {
            console.log('Copy failed: ' + error);
        } else {
            //adapter.log.info(JSON.stringify(results));
            console.log('Copied ' + results.length + ' files to ' + path.join(rootDir, 'node_modules'));
        }
    ...
    //        npmInstall('iobroker.js-controller@stable', false, function () {
                ...
                if (!fs.existsSync(path.join(rootDir, 'iobroker-data', 'iobroker.json'))) {
                    if (fs.existsSync(path.join(jsDir, 'conf', 'iobroker-dist.json'))) {
                        ...
                        console.log('creating conf/iobroker.json as ' + tools.getConfigFileName());
